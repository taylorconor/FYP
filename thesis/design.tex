The location proof system I propose allows \textit{mobile nodes} to create location proofs for each other after physically meeting. When two mobile nodes are in close physical proximity, they initiate a transaction over a short range, ad-hoc network such as Bluetooth.
...
\begin{itemize}
	\item[] \textbf{(1)} Before the transaction begins, both nodes generate a new entry in their nonce list, and generate a new identity from that nonce.
	...
	\item[] \textbf{(8)} Both nodes generate their own transaction $T$, using their alibi's Key Packets. These are then published onto the public blockchain.
\end{itemize}
